<x-layout.admin.app title="Editar Visitante" class="overflow-y-auto max-h-[80vh]">
    <div class="flex items-center justify-between">
        <div class="mb-2">
            <h1 class="text-2xl font-bold">Editar Visitante</h1>
            <p class="mt-1 text-sm">Atualize os dados do visitante {{ $visitante->user->name }}.</p>
        </div>
    ...
    <x-ui.card class="mt-6">
        <form action="{{ route('admin.visitantes.update', $visitante) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6 border-b pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                        ...
                        <div class="flex-shrink-0 mb-4 sm:mb-0">
                            <div id="foto-preview"
                                ...
                                @if($visitante->user->foto)
                                    <img src="{{ Storage::url($visitante->user->foto) }}" alt="Foto do visitante" class="h-full w-full object-cover">
                                @else
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                        <x-forms.input name="nome" label="Nome Completo" :value="old('nome', $visitante->user->name)" required
                            :error="$errors->first('nome')" />
                        
                        <x-forms.input name="cpf" label="CPF" :value="old('cpf', $visitante->user->cpf)" required 
                            :error="$errors->first('cpf')" id="cpf" placeholder="000.000.000-00" />
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <x-forms.input name="email" type="email" label="E-mail" :value="old('email', $visitante->user->email)"
                        :error="$errors->first('email')" />
                    <x-forms.input name="telefone" label="Telefone" :value="old('telefone', $visitante->user->telefone)" 
                        :error="$errors->first('telefone')" placeholder="(00) 00000-0000" id="telefone" />
                    <x-forms.input name="data_nascimento" label="Data de Nascimento" 
                        :value="old('data_nascimento', $visitante->user->data_nascimento ? \Carbon\Carbon::parse($visitante->user->data_nascimento)->format('Y-m-d') : '')"
                        :error="$errors->first('data_nascimento')" type="date" />
                        <x-forms.select name="ativo" label="Status" :value="old('ativo', $visitante->ativo)" required 
            ...
            <div class="mb-6 border-b pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-forms.select name="morador_responsavel_id" label="Visitar o Morador" 
                        :value="old('morador_responsavel_id', $visitante->morador_responsavel_id)" required 
                        :error="$errors->first('morador_responsavel_id')">
                        <option value="">Selecione um morador</option>
                        @foreach ($moradores as $morador)
                            <option value="{{ $morador->id }}"
                                {{ old('morador_responsavel_id', $visitante->morador_responsavel_id) == $morador->id ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-forms.input name="data_validade_inicio" label="Data de Início" 
                        ...
                        required :error="$errors->first('data_validade_inicio')" type="date" />
                    
                    <x-forms.input name="data_validade_fim" label="Data de Término (opcional)" 
                    ...
                    <div class="md:col-span-2">
                        <x-forms.checkbox name="recorrente" label="Visita Recorrente" :value="true"
                            :checked="old('recorrente', $visitante->recorrente)" id="recorrente" />
                    </div>

                    <div id="recorrencia-campos" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border rounded-md {{ old('recorrente', $visitante->recorrente) ? '' : 'hidden' }}">
                        <div>
                            ...
                            <div class="space-y-2">
                                ...
                                @endphp
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="0" id="dia-0" {{ in_array('0', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-0" class="ml-2 text-sm">Domingo</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="1" id="dia-1" {{ in_array('1', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-1" class="ml-2 text-sm">Segunda-feira</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="2" id="dia-2" {{ in_array('2', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-2" class="ml-2 text-sm">Terça-feira</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="3" id="dia-3" {{ in_array('3', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-3" class="ml-2 text-sm">Quarta-feira</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="4" id="dia-4" {{ in_array('4', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-4" class="ml-2 text-sm">Quinta-feira</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="5" id="dia-5" {{ in_array('5', $diasSelecionados) ? 'checked' : '' }}>
                                    <label for="dia-5" class="ml-2 text-sm">Sexta-feira</label>
                                </div>
                                <div>
                                    <input type="checkbox" name="dias_semana[]" value="6" id="dia-6" {{ in_array('6', $diasSelecionados) ? 'checked' : '' }}>
                        ...
                        <div>
                            <x-forms.input name="horario_inicio" label="Horário de Início" 
                            ...
                            <x-forms.input name="horario_fim" label="Horário de Término" 
