<x-layout.admin.app title="Editar Morador" class="overflow-y-auto max-h-[80vh]">
    ...
    <div class="flex items-center justify-between">
        <div class="mb-2">
            ...
            <p class="mt-1 text-sm">Preencha os dados para editar o morador.</p>
        </div>
    ...
    <x-ui.card class="mt-6">
        <form action="{{ url('admin/moradores/' . $morador->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="w-full px-5">
                ...
                <div class="mb-6">
                    <h2 class="text-lg font-semibold mb-3 border-b pb-2">Dados Pessoais</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
                            <div class="flex-shrink-0 mb-4 sm:mb-0">
                                <div id="foto-preview"
                                    ...
                                    @if (isset($dados['user']['foto']))
                                        <img src="{{ Storage::url($dados['user']['foto']) }}" alt="Foto do morador"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                            <x-forms.input name="nome" label="Nome Completo" :value="old('nome', $dados['user']['name'])" required
                                :error="$errors->first('nome')" />

                            <x-forms.checkbox name="is_responsavel" label="Responsável pelo apartamento" :value="true"
                                :checked="old('is_responsavel', $isResponsavel)" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <h2 class="text-lg font-semibold mb-3 border-b pb-2">Informações Pessoais</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-forms.input name="cpf" label="CPF" :value="old('cpf', $dados['user']['cpf'])" required :error="$errors->first('cpf')"
                            id="cpf" placeholder="000.000.000-00" />
                        <x-forms.input name="email" type="email" label="E-mail" required :value="old('email', $dados['user']['email'])"
                            :error="$errors->first('email')" />
                        <x-forms.input name="telefone" label="Telefone" :value="old('telefone', $dados['user']['telefone'])" required :error="$errors->first('telefone')"
                            placeholder="(00) 00000-0000" id="telefone" />
                        <x-forms.input name="data_nascimento" label="Data de Nascimento" :value="old(
                            'data_nascimento',
                            $dados['user']['data_nascimento']
                                ? \Carbon\Carbon::parse($dados['user']['data_nascimento'])->format('Y-m-d')
                                : '',
                        )" required
                ...
                <!-- Seção: Informações de Residência -->
                <div class="mb-6">
                        ...
                        <x-forms.select name="condominio_id" label="Condomínio" :value="old('condominio_id', $dados['condominio']['id'])" required
                            ...
                            @foreach ($condominios as $condominio)
                                <option value="{{ $condominio->id }}"
                                    {{ old('condominio_id', $dados['condominio']['id']) == $condominio->id ? 'selected' : '' }}>
                                    {{ $condominio->nome }}
                                </option>
                            @endforeach
                        </x-forms.select>
                        ...
                        <x-forms.select name="torre_id" label="Torre" :value="old('torre_id', $dados['torre']['id'])" required :error="$errors->first('torre_id')">
                            ...
                            @foreach ($torres as $torre)
                                <option value="{{ $torre->id }}"
                                    {{ old('torre_id', $dados['torre']['id']) == $torre->id ? 'selected' : '' }}>
                                    {{ $torre->nome }}
                                </option>
                            @endforeach
                        </x-forms.select>

                        <x-forms.input name="apartamento" label="Apartamento" :value="old('apartamento', $dados['apartamento']['numero'])" required
                            :error="$errors->first('apartamento')" placeholder="Ex: 101, 202, etc." />

                        <x-forms.select name="status" label="Status" :value="old('status', $morador->ativo ? 'ativo' : 'inativo')" required :error="$errors->first('status')">
                            <option value="ativo"
                                {{ old('status', $morador->ativo ? 'ativo' : 'inativo') == 'ativo' ? 'selected' : '' }}>
                                Ativo
                            </option>
                            <option value="inativo"
                                {{ old('status', $morador->ativo ? 'ativo' : 'inativo') == 'inativo' ? 'selected' : '' }}>
                                Inativo</option>
                        </x-forms.select>
                    </div>
                </div>
